...
type OpticalDrive struct {
	gorm.Model
	 ...
	 Bd float64 `json:"bd" form:"bd"`
	 Dvd float64 `json:"dvd" form:"dvd"`
	 Cd float64 `json:"cd" form:"cd"`
...
func (OpticalDrive)ImportData(db *gorm.DB) error {
	...
	for i := range records {
		...
		m := OpticalDrive{
			...
			Bd: utils.ToFloat64(records[i][2]),
			Dvd: utils.ToFloat64(records[i][3]),
			Cd: utils.ToFloat64(records[i][4]),
			BdWrite: records[i][5],
